...
public class DataHoras {
	...
	public Hora getHorasDelIntegrante(Integrante i) 
	{
		...
		try 
		{
			stmt = DbConnector.getInstancia().getConn().prepareStatement("select * FROM hora WHERE idIntegrante = ? ORDER BY inicio desc limit 1");
	...
	public void add (Hora hr) 
	{
		...
		try 
		{
			stmt=DbConnector.getInstancia().getConn().
					prepareStatement("insert into hora(idIntegrante, inicio) values(?,?)");
			stmt.setInt(1, hr.getIdIntegrante());
	...
	public void update(Hora hr)
	{
		...
		try 
		{
			stmt=DbConnector.getInstancia().getConn().prepareStatement("update hora set fin = ?, horasJugadas = ? where idIntegrante=? and inicio=?");
	...
	public void remove(Hora hr) 
	{
		...
		try 
		{
			stmt=DbConnector.getInstancia().getConn().prepareStatement("delete from hora where idIntegrante = ? and inicio = ? and fin is null");
			stmt.setInt(1, hr.getIdIntegrante());	
			stmt.setObject(2, hr.getInicio());
			stmt.executeUpdate();
		} 
	...
	public void deleteHorasByIntegrante(Integrante i) 
	{
		...
		try 
		{
			stmt=DbConnector.getInstancia().getConn().prepareStatement("delete from hora where idIntegrante = ?");
			stmt.setInt(1, i.getIdIntegrante());
			stmt.executeUpdate();
		} 
		catch (SQLException e) 
	...
	public LinkedList<Hora> getTuplasIntegrante(int id, LocalDate fecha, LocalDate fechaFin) 
	{
		...
		try 
		{
			stmt=DbConnector.getInstancia().getConn().prepareStatement("select horasJugadas FROM horas WHERE idIntegrante = ? and fecha BETWEEN ? and ? and horaFin is not null ");
			stmt.setInt(1, id);
	...
	public LinkedList<Hora> getHorasAllIntegrantes(LocalDate fecha, LocalDate fechaFin) 
	{
        ...
        try 
        {
            stmt=DbConnector.getInstancia().getConn().prepareStatement("SELECT idIntegrante, time_format(sum(horasJugadas), \"%H:%i:%S\") as horasJugadas  from horas \n"
